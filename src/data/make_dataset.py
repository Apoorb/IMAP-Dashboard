...
def read_crash_dat(crash_file_):
    ...
    print(f"Crash Data cooridnate sytem is {crash_gdf.crs.srs}")
    crash_gdf_.columns = [
        inflection.underscore(col_name) for col_name in crash_gdf.columns
    ]
    return crash_gdf_
...
def fix_crash_dat_type(crash_df_, max_highway_class=3):
    ...
    """
    crash_df_fil_ = crash_df_.assign(
        ...
        ka_cnt=lambda df: pd.to_numeric(df.ka_cnt, errors="coerce").astype(
            pd.Int32Dtype()
        ),
        bc_cnt=lambda df: pd.to_numeric(df.bc_cnt, errors="coerce").astype(
            pd.Int32Dtype()
        ),
        pdo_cnt=lambda df: pd.to_numeric(df.pdo_cnt, errors="coerce").astype(
            pd.Int32Dtype()
        ),
        total_cnt=lambda df: pd.to_numeric(df.total_cnt, errors="coerce").astype(
            pd.Int32Dtype()
        ),
        ...
        lambda df: df.route_class <= max_highway_class
    ]
...
def test_crash_dat(crash_df_fil_):
    ...
    """
    assert (crash_df_fil_.route_county == crash_df_fil_.county).all(), (
        "County number in the data does not matches county number from route_gis."
    )
def create_new_crash_fields(crash_df_fil_, ka_si_factor=76.8, bc_si_factor=8.4, ou_si_factor=1):
    crash_df_fil_si_ = crash_df_fil_.assign(
        severity_index= lambda df, ka_si=ka_si_factor, bc_si=bc_si_factor,
                               ou_si=ou_si_factor: (ka_si * df.ka_cnt + bc_si * df.bc_cnt + ou_si * df.pdo_cnt) / df.total_cnt
    )
...
if __name__ == "__main__":
    ...
    crash_gdf = read_crash_dat(
        crash_file_ = crash_file)
